---
name: ollama-prompting
description: Internal guidance for composing prompts for the local model for coding, review, diagnosis, and research tasks inside the Ollama Claude Code plugin
user-invocable: false
---

# Prompting the local model

Use this skill when `ollama:ollama-rescue` needs to ask the local model for help.

Prompt the local model like an operator, not a collaborator. Keep prompts compact and block-structured with XML tags. State the task, the output contract, the follow-through defaults, and the small set of extra constraints that matter.

Core rules:
- Prefer one clear task per run. Split unrelated asks into separate runs.
- Tell the local model what done looks like. Do not assume it will infer the desired end state.
...
- `<default_follow_through_policy>`: what the local model should do by default instead of asking routine questions.
...
- Write-capable tasks: add `action_safety` so the local model stays narrow and avoids unrelated refactors.
...
- Use `task --resume-last` for follow-up instructions on the same thread. Send only the delta instruction instead of restating the whole prompt unless the direction changed materially.
...
- Ask the local model for brief, outcome-based progress updates only when the task is long-running or tool-heavy.
...
3. Decide whether the local model should keep going by default or stop for missing high-risk details.
