...
import { buildProviderArgs } from "./app-server.mjs";
...
function formatCodexResumeCommand(job) {
  ...
  return `codex ${buildProviderArgs().join(" ")} resume ${job.threadId}`;
}

function appendActiveJobsTable(lines, jobs) {
  lines.push("Active jobs:");
  lines.push("| Job | Kind | Status | Phase | Elapsed | Codex CLI Session ID | Summary | Actions |");
  lines.push("| --- | --- | --- | --- | --- | --- | --- | --- |");
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if (job.threadId) {
    lines.push(`  Codex CLI session ID: ${job.threadId}`);
  }
  const resumeCommand = formatCodexResumeCommand(job);
  if (resumeCommand) {
    lines.push(`  Resume with the Codex CLI: ${resumeCommand}`);
  }
...
export function renderStoredJobResult(job, storedJob) {
  ...
  const resumeCommand = threadId ? `codex ${buildProviderArgs().join(" ")} resume ${threadId}` : null;
  if (isStructuredReviewStoredResult(storedJob) && storedJob?.rendered) {
    const output = storedJob.rendered.endsWith("\n") ? storedJob.rendered : `${storedJob.rendered}\n`;
    if (!threadId) {
      return output;
    }
    return `${output}\nCodex CLI session ID: ${threadId}\nResume with the Codex CLI: ${resumeCommand}\n`;
  }
  ...
  if (threadId) {
    lines.push(`Codex CLI session ID: ${threadId}`);
    lines.push(`Resume with the Codex CLI: ${resumeCommand}`);
  }
