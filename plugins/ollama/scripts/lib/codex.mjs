...
async function requestExternalAgentSessionImport(client, params) {
  ...
  timeout = setTimeout(() => {
    rejectCompleted(new Error("Timed out waiting for the Codex CLI to finish importing the Claude session."));
...
function buildAppServerAuthStatus(accountResponse, configResponse) {
  ...
  if (requiresOpenaiAuth === false) {
    return buildAuthStatus({
      loggedIn: true,
      detail: `${providerLabel} is configured and does not require a login`,
      source: "app-server",
      requiresOpenaiAuth,
      provider: providerId
    });
  ...
  return buildAuthStatus({
    loggedIn: false,
    detail: `${providerLabel} requires a login`,
    source: "app-server",
    requiresOpenaiAuth,
    provider: providerId
  });
...
export async function importExternalAgentSession(cwd, options = {}) {
  ...
  return withDirectAppServer(cwd, async (client) => {
    emitProgress(options.onProgress, "Importing Claude session into the Codex CLI.", "transferring");
    try {
      ...
      if (error?.rpcCode === -32601) {
        throw new Error(
          "This Codex CLI version does not support Claude session transfer. Update the Codex CLI with `npm install -g @openai/codex@latest`, then retry.",
          { cause: error }
        );
      }
      throw error;
    ...
    if (!threadId) {
      ...
      throw new Error(
        `The Codex CLI reported that the Claude import completed, but did not record an imported thread.${stderr ? `\n${stderr}` : " Check the Codex app-server logs for the underlying import error."}`
      );
    }
