...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/ollama-companion.mjs transfer [--source <claude-jsonl>] [--json]",
...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (!codexStatus.available) {
    nextSteps.push("Install the Codex CLI with `npm install -g @openai/codex`.");
  }
...
function renderTransferResult(payload) {
  const lines = [
    "Transferred the Claude session into a Codex CLI thread with visible turn history.",
    `Codex CLI session ID: ${payload.threadId}`,
    `Resume with the Codex CLI: ${payload.resumeCommand}`
  ];
  return `${lines.join("\n")}\n`;
}
...
async function executeTransfer(cwd, options = {}) {
  ...
  const payload = {
    threadId: result.threadId,
    resumeCommand: `codex ${buildProviderArgs().join(" ")} resume ${result.threadId}`,
    sourcePath,
