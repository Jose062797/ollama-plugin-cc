...
Invoke the `ollama:codex-rescue` subagent via the `Agent` tool (`subagent_type: "ollama:codex-rescue"`), forwarding the raw user request as the prompt.
`ollama:codex-rescue` is a subagent, not a skill — do not call `Skill(ollama:codex-rescue)` (no such skill) or `Skill(ollama:rescue)` (that re-enters this command and hangs the session). The command runs inline so the `Agent` tool stays in scope; forked general-purpose subagents do not expose it.
...
- If the request includes `--background`, run the `ollama:codex-rescue` subagent in the background.
- If the request includes `--wait`, run the `ollama:codex-rescue` subagent in the foreground.
