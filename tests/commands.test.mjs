...
test("rescue command absorbs continue semantics", () => {
  ...
  const readme = fs.readFileSync(path.join(ROOT, "docs", "UPSTREAM_README.md"), "utf8");
  ...
  // request to the `ollama:codex-rescue` subagent") while running under
  ...
  assert.match(rescue, /subagent_type: "ollama:codex-rescue"/);
  assert.match(rescue, /do not call `Skill\(ollama:codex-rescue\)`/i);
  ...
  assert.match(rescue, /run the `ollama:codex-rescue` subagent in the background/i);
  ...
  assert.match(runtimeSkill, /use the `ollama-prompting` skill to rewrite the user's request into a tighter Codex prompt/i);
  ...
  assert.match(readme, /`ollama:codex-rescue` subagent/i);
...
test("internal docs use task terminology for rescue runs", () => {
  ...
  const promptRecipes = read("skills/ollama-prompting/references/codex-prompt-recipes.md");
